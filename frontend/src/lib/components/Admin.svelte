<script>
  ...
  let expanded = {};    // map: submission_id -> bool (YAML preview)
  let denyTarget = null; // submission row currently being denied
  let denyReason = '';
  ...
  // Pull `has_dangerous_files` + `dangerous_files` out of the stored YAML
  // so the admin can see the scan result without expanding the full YAML
  // or clicking through to the PR.
  function parseDangerousFromYAML(yaml) {
    ...
    for (const raw of yaml.split('\n')) {
      ...
      if (item) {
        ...
        break; // a non-indented non-blank line ends the list
      }
    }
      ...
      <div class="space-y-2">
        {#each submissions as s}
          ...
          <div class="bg-bg-secondary border border-border rounded-lg p-4">
            ...
            <!-- Dangerous-file scan result (parsed from stored YAML) -->
            {#if danger.has}
