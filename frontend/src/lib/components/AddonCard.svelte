<script>
  import { showNotification, warningAddon, showWarningModal, downloadProgress, uninstallAddon, showUninstallConfirm, selectedAuthor, showAuthorModal } from '../stores/app.js';
  import { DownloadAddon } from '../../../wailsjs/go/main/App.js';

  export let addon;

  // 1234 → "1.2k", 1234567 → "1.2M", small numbers stay as-is.
  ...
  async function handleDownload(e) {
    ...
    // Re-check the gates the details modal also enforces, so users who
    // skip the modal and click the row's inline install icon get the
    // same blocking behaviour (and same explanation).
    ...
    // Already-installed addons: the inline button is "Reinstall" / Update,
    // skip the missing-dep gate (we never block uninstall, and reinstall
    // implies the addon's already on disk so deps are presumably handled).
  ...
  function openAuthor(e) {
    e.stopPropagation(); // don't also fire the card's row-click
...
>
  <!-- Addon Icon — always shows the author's icon (or a generic) so the
       danger badge can live next to the name where it actually grabs attention. -->
    ...
    <div class="flex items-center gap-2">
      ...
      {#if addon.has_dangerous_files}
        <svg class="w-4 h-4 text-warning flex-shrink-0" viewBox="0 0 24 24" fill="currentColor" aria-label="Contains dangerous files">
          <title>Contains dangerous files (.bat / .exe / .ps1 / .cmd / .vbs)</title>
