    ...
    <div class="bg-bg-secondary border border-border rounded-2xl max-w-2xl w-full max-h-[85vh] overflow-hidden flex flex-col shadow-modal" transition:modalContent>
      ...
      <div class="px-6 py-5 border-b border-border bg-header-grad flex justify-between items-start gap-4">
        <div class="flex items-center gap-4 min-w-0">
          <div class="relative w-16 h-16 rounded-2xl bg-gradient-to-br from-bg-tertiary to-bg-secondary flex items-center justify-center flex-shrink-0 overflow-hidden ring-1 ring-border">
            ...
            {#if $selectedAddon.is_installed}
              <span class="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-accent rounded-full ring-2 ring-bg-secondary flex items-center justify-center">
          ...
          <div class="min-w-0">
            <div class="flex items-baseline gap-3 flex-wrap">
              <h2 class="text-xl font-bold text-text-primary truncate tracking-tight">{$selectedAddon.name}</h2>
              <span class="text-xs font-mono text-text-muted flex-shrink-0">v{$selectedAddon.version}</span>
            </div>
            <p class="text-sm text-text-secondary mt-1">
              by
              <button
                on:click={openAuthor}
                class="text-text-secondary hover:text-accent hover:underline transition-colors font-medium"
        ...
        <button
          on:click={close}
          class="p-2 hover:bg-bg-tertiary rounded-lg transition-colors flex-shrink-0 text-text-muted hover:text-text-primary"
        >
