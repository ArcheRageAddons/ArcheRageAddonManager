<script>
  ...
  // localStorage key for the user's preferred view mode. Read once on
  // module load — the value is cheap to fetch and doesn't change without
  // the toggle below explicitly setting it.
  ...
  async function handleRefresh() {
    ...
    try {
      await RefreshAddons();
      allAddons = (await GetAddons()) || [];
      // Re-count the bell's pending updates against the just-refreshed
      // registry data. Direct call instead of a window event — installSerially
      // already covers the bulk-install case via DownloadProgress's
      // `addon-installed` emission, so the bell only needs to listen for
      // that one signal.
...
<div class="h-full flex flex-col overflow-hidden">
  ...
  <div class="flex items-center gap-3 p-4 pr-16 border-b border-border bg-bg-secondary">
    ...
    <!-- View-mode toggle. Preference persisted to localStorage so the
  ...
  <div class="flex-1 overflow-y-auto p-4">
    ...
    {:else if viewMode === 'grid'}
      <!-- Grid: auto-fit columns sized to ~150px wide, gap between tiles. -->
