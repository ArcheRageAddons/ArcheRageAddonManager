<script>
  ...
  import { showNotification, uninstallAddon, showUninstallConfirm, waitForDownloadComplete, downloadProgress } from '../stores/app.js';
  import { GetInstalledAddons, DownloadAddon } from '../../../wailsjs/go/main/App.js';
  ...
  onMount(() => {
    ...
    // Listen for addon changes (install/uninstall/update)
  ...
  async function handleUpdate(addon) {
    try {
      await DownloadAddon(addon.id);
      showNotification(`${addon.name} updated successfully!`, 'success');
      await loadInstalled();
    } catch (e) {
      showNotification(`Failed to update: ${e}`, 'error');
    }
  }
  ...
  async function handleBulkUpdate() {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      // Drive the global DownloadProgress modal so the user sees
      // per-addon progress instead of just a header counter.
      downloadProgress.set({
        isDownloading: true,
        addonId: addon.id,
        addonName: targets.length > 1 ? `${addon.name} (${i + 1}/${targets.length})` : addon.name,
        current: 0,
        total: 100,
      ...
      try {
        ...
        const result = await waitForDownloadComplete(addon.id);
        if (!result.success) failed++;
      } catch (e) {
        console.error('bulk update failed for', addon.id, e);
        failed++;
      }

      // Wait for DownloadProgress's post-complete reset to flush before
      // we set the next iteration's state (otherwise its 500ms-delayed
      // setTimeout would clobber our next update). See the same dance
      // in AddonDetailsModal.installMissingDeps.
...
<div class="h-full flex flex-col overflow-hidden">
  <!-- Header -->
  <div class="flex justify-between items-center p-4 border-b border-border bg-bg-secondary">
