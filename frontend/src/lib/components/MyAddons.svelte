<script>
  ...
  // ---------------------------------------------------------------------
  // Derive "Your addons" — one entry per slug, pointing at the most recent
  // approved submission. This is the canonical "what's currently live" view.
  // ---------------------------------------------------------------------
  $: yourAddons = (() => {
    const map = {};
  ...
  // IDs we always keep around so the Update flow keeps working — the latest
  // approved per slug for each addon.
  $: keepIds = new Set(yourAddons.map((a) => a.id));

  // History entries the user is allowed to delete: not pending, and not the
  // latest-approved-per-slug.
  function canDelete(s) {
    return s.status !== 'pending' && !keepIds.has(s.id);
  }
  $: deletableIds = submissions.filter(canDelete).map((s) => s.id);

  // Quick YAML field extractors — our YAML schema is flat enough that a
  // regex match is faster + dependency-free vs pulling in a parser.
  ...
  async function update(s) {
    ...
    try {
      // Pass the row data we already have from GetMySubmissions so Go
      // just parses the YAML — no extra Supabase round-trip.
...
<!-- ============== Clear submission history confirmation ============== -->
...
<!-- ============== Scary delete-addon confirmation ============== -->
