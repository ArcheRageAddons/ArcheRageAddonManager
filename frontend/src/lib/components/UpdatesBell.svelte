<script>
  ...
  let open = false;
  let busyById = {};   // per-row spinner: { [id]: true }
  let bulkBusy = false;
  let bulkProgress = { done: 0, total: 0 };

  // Refresh on mount and on every install/uninstall event. The
  // addon-installed event is the leaf signal DownloadProgress.svelte fires
  // on every install completion — including those driven by installSerially
  // — so it covers both manual refresh AND bulk-install completion. Other
  // pages (Browse, Installed) call refreshAvailableUpdates() directly when
  // they refresh, no separate event channel needed.
  ...
  // Close the dropdown when clicking outside or hitting ESC.
  ...
  // Single-row update — installSerially handles the progress-modal
  // orchestration so the user gets the same feedback as any other install.
