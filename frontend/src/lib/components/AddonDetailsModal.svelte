<script>
  ...
  import { selectedAddon, showAddonDetails, showNotification, warningAddon, showWarningModal, downloadProgress, currentUser, uninstallAddon, showUninstallConfirm, selectedAuthor, showAuthorModal, waitForDownloadComplete } from '../stores/app.js';
  import {
    DownloadAddon,
    OpenURL,
    GetMyRating,
  ...
  let downloadSize = null; // bytes; null=not yet fetched, -1=unknown, 0+=actual
  let sizeFetchedFor = null;

  // Whether the primary footer button should act as Uninstall (true) vs
  // Download/Update. Computed reactively so the button label, color, and
  // overlay-base gate all stay in sync.
  $: isInstalledNoUpdate = !!($selectedAddon && $selectedAddon.is_installed && !$selectedAddon.has_update);

  // Install gates. Both block install (Download / Update) but NOT uninstall:
  //   - baseMissing: overlay addon whose base isn't installed yet
  //   - depsMissing: regular addon with at least one un-installed dependency
  ...
  // Split dependencies into installed / missing for the categorised view.
  ...
  // Trigger size fetch when modal opens for a new addon.
  ...
  async function fetchSize(id) {
    try {
      ...
      // Only commit the result if the modal is still on the same addon —
      // user can click rapidly between addons and we don't want a stale
      // size to overwrite the current one.
  ...
  // When the modal opens for a different addon, refresh "your rating" so
  // each open reflects the latest state.
  ...
  function openAuthor() {
    ...
    // Close the addon details modal so the author modal owns the screen.
    close();
  }
  ...
  async function handlePrimaryAction() {
    // Installed addons: primary action is Update (when an update exists)
    // or Uninstall (when already at latest). Fresh installs or addons
    // with updates available go down the download path; everything else
    // routes to the uninstall confirm modal.
    ...
    // Defence-in-depth: the install button is also disabled in the
    // template when deps are missing, but a stale snapshot of the modal
    // could conceivably still fire this handler. Surface the same
    // explanation as the disabled-button tooltip and bail.
  ...
  async function performDownload() {
    ...
    // Start download (async, completion handled by DownloadProgress component)
    DownloadAddon($selectedAddon.id);
  ...
  async function installMissingDeps() {
    ...
    const missing = missingDeps.slice();
    ...
    for (let i = 0; i < missing.length; i++) {
      ...
      // Drive the global DownloadProgress modal — same machinery the
      // single-install path uses. Without this, the user sees no progress
      // feedback while a dep is downloading (could be tens of seconds).
      downloadProgress.set({
        isDownloading: true,
        addonId: dep.name,
        addonName: missing.length > 1 ? `${dep.name} (dependency ${i + 1}/${missing.length})` : dep.name,
        current: 0,
        total: 100,
        message: 'Starting download...'
      });
      ...
      try {
        DownloadAddon(dep.name);
        const result = await waitForDownloadComplete(dep.name);
        if (result.success) ok++; else failed++;
      } catch (e) {
        console.error('dep install failed for', dep.name, e);
        failed++;
      }

      // DownloadProgress's own download:complete handler resets the
      // store ~500ms after success. If we set the next iteration's state
      // before that timer fires, it would clobber our update. Wait for
      // the store to drain before continuing.
    ...
    // Refresh the modal's snapshot of dependencies so the just-installed
    // ones flip from "Missing" to "Installed" without requiring the user
    // to close + reopen the modal. Browse already self-refreshes via the
    // addon-installed event each DownloadAddon emits.
    if ($selectedAddon) {
      try {
        const fresh = await GetAddonDetails($selectedAddon.id);
        if (fresh) selectedAddon.set(fresh);
      } catch (e) {
        // Non-fatal; user can close + reopen to refresh manually.
    ...
    <div class="bg-bg-secondary border border-border rounded-xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col shadow-2xl">
      ...
      <div class="p-5 border-b border-border flex justify-between items-start gap-4">
        <div class="flex items-center gap-4 min-w-0">
          <!-- Icon — same render as AddonCard, larger here. Falls back to
               the generic shape when the YAML has no icon URL or it 404s. -->
      ...
      <div class="p-5 overflow-y-auto flex-1">
        <div class="space-y-4">
          ...
          <!-- Overlay relationship strip — only shown for addons that
               install on top of another (e.g. UI-overhaul patcher). -->
            ...
            <div class="p-4 bg-warning/10 border border-warning/20 rounded-lg flex items-start gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-text-muted mt-1">
                  This addon contains .bat, .ps1, or .exe files. Only install if you trust the author.
                </p>
              </div>
            </div>
          {/if}
