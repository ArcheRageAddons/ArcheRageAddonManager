<script>
  ...
  // Reactive: whenever the modal opens or the author changes, fetch (cached
  // server-side, so flipping between authors is cheap).
  ...
  async function loadAddons() {
    loading = true;
    try {
      // GetAddons returns the full registry list when filters are 'All'/''.
  ...
  // Match by author display name (case-insensitive trim). Unverified
  // free-text from the YAML — same string the user just clicked.
  $: byThisAuthor = (() => {
  ...
  // Surface a verified GitHub login if any of this author's addons carry one.
  // Multiple addons by the same display name SHOULD share a github login;
  // if they don't, that's a sign two different people picked the same name
  // and we just show the first one we see (best-effort, no UI gymnastics).
