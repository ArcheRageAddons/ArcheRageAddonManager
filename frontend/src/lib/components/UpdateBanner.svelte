<script>
  ...
  let update = null;             // { version, url, source_url, body, published_at, asset_url, asset_size }
  let dismissedVersion = null;   // session-only — newer versions still show

  // Self-update flow state.
  //   confirming   — modal asking user to OK the update
  //   downloading  — progress bar visible
  //   done         — short success state before app exits
  ...
  function startUpdate() {
    if (!update) return;
    if (!update.asset_url) {
      // Older release without an .exe asset — fall back to the manual flow.
  ...
  async function confirmAndInstall() {
    ...
    try {
      ...
      // App will exit on success — if we're still here, the Go side
      // already emitted update:download:complete and the phase will
      // flip via the event handler.
    } catch (e) {
      phase = 'error';
...
<!-- Confirm modal — asks the user to OK the swap-and-relaunch. -->
...
<!-- Progress modal — shown during download + swap. No cancel during install
     because rolling back a half-applied swap is messy. -->
...
<!-- Error modal — shows the failure reason and lets the user fall back to
