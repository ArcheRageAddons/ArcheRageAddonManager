<script>
  ...
  onMount(async () => {
    ...
    // Pre-select the first existing candidate, otherwise the current configured path
  ...
  async function handleBrowse() {
    try {
      ...
      if (picked) {
        selectedPath = picked;
        // Insert as the selected entry if it's not already in the list
  ...
  async function handleContinue() {
    ...
    saving = true;
    try {
      // SetAddonPath also marks setup complete; ConfirmAddonPath is for the
      // case where the user is happy with the unchanged auto-detected path.
